    ...
    for (let i = 0; i < 42; i++) {
        ...
        if (isCurrentMonth) {
            const dayEvents = monthEvents.filter(event => {
                ...
                // Filtro de Visibilidade e Usuário:
                let matchesUserAndVisibility = true;

                if (currentUserRole === 'COORDENADOR') {
                    // Coordenador vê todas as tarefas. Se tiver filtrando por usuário, vê só as dele.
                    if (viewingUser !== 'all') {
                        matchesUserAndVisibility = (event.createdBy === viewingUser);
                    }
                } else if (currentUserRole === 'USER') {
                    // Colaborador vê suas próprias tarefas (públicas e privadas) + Tarefas públicas de outros
                    if (event.createdBy === currentUsername) {
                        matchesUserAndVisibility = true;
                    } else {
                        matchesUserAndVisibility = (event.visibility === 'public' || event.visibility !== 'private');
                    }
                } else if (currentUserRole === 'VIEWER') {
                    // Viewer vê apenas tarefas públicas.
                    if (event.visibility === 'private') {
                        matchesUserAndVisibility = false;
                    } else if (viewingUser !== 'all') {
                        matchesUserAndVisibility = (event.createdBy === viewingUser);
                    }
                }

                return matchesDay && matchesIndustry && matchesMatrix && matchesSearch && matchesUserAndVisibility;
            });
