        ...
        if (isCurrentMonth) {
            const dayEvents = monthEvents.filter(event => {
                ...
                if (currentUserRole === 'COORDENADOR') {
                    ...
                    if (viewingUser !== 'all') {
                        matchesUserAndVisibility = (event.createdBy === viewingUser || event.assignedTo === viewingUser || event.createdBy === currentUsername);
                    } else {
                        matchesUserAndVisibility = true;
                    }
                } else if (currentUserRole === 'USER') {
                    // USUÁRIO: Vê APENAS as suas (Criador ou Responsável). NUNCA vê de outros.
...
function openEventDetail(event, fullKey) {
    ...
    if (creatorInfo) {
        ...
        creatorInfo.innerHTML = canSee ? `<b>Criado por:</b> ${getUserDisplayName(event.createdBy)}` : '';
        creatorInfo.style.display = canSee ? 'block' : 'none';
    }
    if (assignedInfo) {
...
document.getElementById('eventForm').onsubmit = function (e) {
    ...
    const newEvent = {
        ...
        division: currentUserDivision, // Salva a divisão para isolamento
...
async function syncWithSheets() {
    ...
    try {
        if (!GOOGLE_SHEETS_WEB_APP_URL || GOOGLE_SHEETS_WEB_APP_URL === '') {
            alert('A sincronização com o Google Sheets ainda não foi configurada.');
            return;
        }

        // 1. Tentar baixar dados e logs da nuvem
        const response = await fetch(GOOGLE_SHEETS_WEB_APP_URL);
        if (!response.ok) throw new Error('Falha na conexão.');

        const data = await response.json();
        let cloudEvents = data.eventsData || {};
        let cloudLogs = data.auditLogs || [];

        // 2. Mesclagem de Eventos
        eventsData = { ...eventsData, ...cloudEvents };

        // 3. Mesclagem de Logs (Auditoria Persistente)
        const localLogs = JSON.parse(localStorage.getItem('eisenhowerAuditLogs')) || [];
        const combinedLogs = [...cloudLogs, ...localLogs].filter((v, i, a) => a.findIndex(t => t.date === v.date && t.taskId === v.taskId) === i);
        localStorage.setItem('eisenhowerAuditLogs', JSON.stringify(combinedLogs.slice(-1000)));

        saveEvents();
        generateCalendar(currentDate);

        // 4. Enviar tudo de volta (Seguro)
        ...
        showToast('Nuvem sincronizada (Tarefas + Auditoria)');
    } catch (err) {
